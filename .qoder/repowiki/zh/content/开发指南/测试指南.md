...
subgraph "前端"
G["src/*"] --> H["package.json"]
H --> I["vite.config.ts"]
end
J["docker-compose.yml"] --> A
J --> G
```
...
- 最佳实践清单
  ...
  - 及时更新Mock与依赖版本。
- 参考文件
  - 后端测试配置与用例：pytest.ini、conftest.py、test_api.py、test_parser.py
