...
## 简介
本文件面向Talos前端Vue3项目的组件与页面开发，聚焦基于Element Plus的UI使用规范、自定义组件开发约定、核心业务组件（资产表单对话框、富文本编辑器、漏洞表单面板、重测面板等）的实现与用法、页面级组件的组织方式（仪表盘、资产列表、漏洞管理等）、状态管理（Pinia store）与数据流、路由配置与导航逻辑、组件通信与事件处理、样式定制与主题配置、响应式设计与移动端适配最佳实践。文档力求在技术深度与可读性之间取得平衡，帮助初学者快速上手，同时为有经验的开发者提供可追溯的代码级参考。

**更新** 本项目已完成现代化架构升级，采用pnpm作为包管理器，新增暗色主题支持，并扩展了多个UI组件以提升用户体验。
...
- src/stores/theme.ts：主题管理Store（新增）
- src/api/client.ts：HTTP客户端封装（请求拦截、错误处理、统一响应）
- src/components：通用业务组件（如资产表单对话框、富文本编辑器、漏洞表单面板、重测面板等）
- src/views：页面级视图（仪表盘、资产列表、漏洞管理等）
- src/utils：工具函数（颜色、格式化等）
- package.json：依赖与脚本
- pnpm-lock.yaml：pnpm锁定文件（新增）
...
B --> J["utils/colors.ts<br/>工具函数"]
K["package.json<br/>依赖管理"] --> A
L["pnpm-lock.yaml<br/>包锁定"] --> K
```
...
## 核心组件
本节聚焦关键业务组件：资产表单对话框、富文本编辑器、漏洞表单面板和重测面板。它们体现了Element Plus的使用范式与自定义组件的开发约定。
...
- 漏洞表单面板（VulnFormPanel.vue）[新增]
  - 职责：专门用于漏洞信息的录入和编辑，支持复杂的表单结构和动态字段
  - 特性：支持漏洞等级选择、影响范围评估、修复建议填写
  - 集成：与漏洞详情页面无缝衔接，支持实时预览

- 重测面板（VulnRetestPanel.vue）[新增]
  - 职责：提供漏洞重测功能界面，支持测试参数配置和结果对比
  - 功能：测试环境选择、扫描策略配置、历史结果对比
  - 交互：支持批量重测和进度跟踪
...
VulnList --> VulnForm["VulnFormPanel.vue"]
VulnList --> Retest["VulnRetestPanel.vue"]
...
C2["RichEditor.vue"]
C3["VulnFormPanel.vue"]
C4["VulnRetestPanel.vue"]
end
subgraph "状态层"
S1["auth.ts"]
S2["theme.ts"]
...
- 常见问题
  ...
  - pnpm依赖问题：检查pnpm-lock.yaml与node_modules一致性
- 调试技巧
  - 使用浏览器开发者工具查看网络请求与响应
  - 打印Store状态变化与组件生命周期
...
## 结论
Talos前端采用清晰的Vue3分层架构，结合Element Plus与Tailwind CSS构建高效、可维护的用户界面。通过Pinia统一管理状态，组件间通信简洁明确，路由与导航逻辑清晰。核心业务组件（资产表单对话框、富文本编辑器、漏洞表单面板、重测面板）提供了良好的复用性与扩展性。项目已完成现代化升级，采用pnpm包管理器提升依赖管理效率，新增暗色主题支持改善用户体验。遵循本文档的规范与实践，可进一步提升开发效率与用户体验。
