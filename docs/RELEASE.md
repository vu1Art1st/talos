...
- **报告「草稿 / 已定稿」状态功能整体移除**（2026-09-23）：该状态不参与权限、可见性、可编辑性、
  列表筛选与统计，也不参与复测前置校验，仅由「导出 Word 成功后定稿」与「已定稿报告内容变更回退草稿」
  两条规则驱动，已无实际需求。移除范围：`constants.py` 的 `ReportStatus` 枚举及名称 / 色值字典、
  导出成功后的定稿与保存时的回退逻辑（`workers/main.py`、`api/v1/reports.py`，含只服务于回退判断的
  `_report_content_changed`）、`reports.status` 列（迁移 `b9c0d1e2f3a4`）、`/meta` 的 `report_status`
  名称与色值字典、报告编辑器的状态下拉与工单列表 / 流程抽屉的报告状态标签、前端 `Report.status` 类型
  与 `colors.ts` 注册表项。**升级即删列**：存量 `draft` / `final` 取值不再保留（不可还原）。
...
- **报告维度复测三态恒为「未发起复测」**（2026-09-22，还原库实测定位）：渗透测试工单的「关联报告」
  与工单流程抽屉里，工单内**所有初测报告**一律标「未发起复测」（复测报告因标题含「复测」显示正常）。
  根因是判据单源：`triggered` 只认轮次 `src_report_id`，而实测存量 60 条轮次中仅 3 条有它 ——
  43 条「报告导入复测」与 1 条「手动流转至复测中」路径**从不写源报告**（`import_service` 建轮时未带），
  13 条可回填的「报告《X》发起复测」轮次则因升级流程该回填步骤未生效而残留为空。
  （附带证伪了「漏洞均未修复导致」的推测：`all_closed` 只在 `triggered` 成立后区分 复测中/复测完成，
  且已闭环工单里初测报告的章节漏洞本就全部为已修复。）
  修复分三层：
  1. 读取侧新增 `plan_service.retest_triggered_report_ids`（`fill_report_closure` 与
     `report_retest_state_map` 共用）：`src_report_id` 权威直连 **∪** 无源轮次按「本轮复测报告章节漏洞
     ∩ 本报告章节漏洞」代偿（代偿只作用于无权威关联的轮次，避免把复测发起之后新建、复用同一漏洞的报告误标）；
  2. 写入侧「报告导入复测」入库收尾补写 `src_report_id`（`plan_service.infer_src_report_id` 推断：
     同工单、标题不含「复测」、交集最大且唯一）；
  3. `scripts/backfill_retest_src_report.py` 增加同一推断口径（实测可回填 13 → 50 条）。
  效果（还原库实测 125 份报告）：判为已发起复测由 3（仅权威关联）增至 **94**；工单内有轮次的报告里
  「未发起复测」由 38 降至 **19**，其余仍为 none 且均属正确（无漏洞报告 / 无轮次 / 仅手动流转的工单）。
  `all_closed` 口径（复测完成 = 本报告章节漏洞全部闭环）与工单级口径的差异保持不变。
- **`upgrade.sh` 的 fail-open 步骤改为统一显式告警**（2026-09-22，由上一条事故倒逼）：原各步骤用
  `|| echo "（…失败，可稍后手动执行：…）"` 兜底，单行提示极易被后续输出淹没——本次事故正是这样静默了
  一轮数据回填。现改为 `record_warning "<步骤>" "<影响说明>" "<手动补救命令>"` 统一登记（脚本 46-57 行），
  覆盖升级前备份失败、构建缓存清理失败、两个回填步骤失败，并在**升级末尾**汇总打印
  「有 N 项维护步骤失败」+ 逐条列出（153-162 行）并走 `notify.sh` 渠道通知；硬依赖（`git pull` /
  镜像构建 / 数据库迁移）仍保持失败即中止。行为仍为 **fail-open**：不因维护步骤失败阻断 `compose up -d`。
