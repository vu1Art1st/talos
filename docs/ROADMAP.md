...
**现状基线**：2.18.1（2026-09-19）——安全审计确认的 6 项漏洞（TALOS-2026-001~006）全部修复并落地批次 A~E 加固（附件路径白名单、出站目标白名单、复测标题转义、报告导出只内嵌本地图片、图片需登录下发、refresh 令牌轮换、可信代理层数、解压配额、首登改密服务端强制、Nginx 安全响应头）；中期功能 F3 通知渠道 / F4 CVSS 计算器 / F6 开放 API / F7 审计已落地，会话令牌改为空闲 24 小时滑动过期；工单复测完成判定已改为**工单级口径**（2.17.1）；影响URL 批量粘贴自动切分与「套用模板」跨模板全局搜索已发布（2.18.0）；代码质量审计（A~F 各项）已收口，静态检查门禁（后端 `ruff`+`vulture`、前端 `typecheck`+`test`+`build`）、命名域规则、验收口径与文档治理规则均已成文于 `AGENTS.md`；F2 扫描器报告接入与 F5 定期报表订阅已取消（暂无计划）。
